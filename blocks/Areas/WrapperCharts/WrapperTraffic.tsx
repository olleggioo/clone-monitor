...
const WrapperTraffic: FC<any> = ({
    ...
    const fetchRamData = async () => {
        try {
            const res = await deviceAPI.getBoxNetBytes({
                where: {
                    appId: id,
    ...
    return <>
        ...
        {ramData && ramData.length > 0 && <ChartTraffic cpuCoreData={ramData} />}
    </>
}

export default WrapperTraffic;