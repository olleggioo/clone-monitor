...
const UpdateManyMiningState: FC<{
    correctState?: any
    state?: any
    onClose?: any
    ...
    const handleSubmit = () => {
        const devicesIds = state.flatMap((item: any) => item.devicesId.map((device: any) => ({ id: device.id }))) 
        if(state && state.length !== 0) {
            const devicesIds = state.flatMap((item: any) => item.devicesId.map((device: any) => ({ id: device.deviceId }))) 
            ...
            if(selectedOption.value === true) {
                enableCorrectDevices(correctState)
            }
            if(selectedOption.value === false) {
                disableCorrectDevices(correctState)
            }
        }
    }
...
export default UpdateManyMiningState